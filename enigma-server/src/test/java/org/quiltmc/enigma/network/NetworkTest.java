...
public class NetworkTest {
	...
	private static TestEnigmaClient connectClient(ClientPacketHandler handler, TestEnigmaServer server) throws IOException {
	...
	public void testLogin(RepetitionInfo repetitionInfo) {
		testServer(repetitionInfo.getCurrentRepetition(), server -> {
			Assertions.assertTrue(server.getClients().isEmpty());

			var handler = new DummyClientPacketHandler();
			var client = connectClient(handler, server);
			...
			final boolean confirmed = server
					.waitChangeConfirmation(server.getClients().get(0), 1)
			...
			client.disconnect();
			Assertions.assertTrue(client.socket.isClosed());
		});
	}
	...
	public void testTakenUsername() {
		testServer(0, server -> {
			...
			var client2 = connectClient(handler2, server);
	...
	@Test
	public void testUnapprovedMessage() {
		testServer(0, server -> {
			var handler = new DummyClientPacketHandler();
			var client = connectClient(handler, server);
			handler.client = client;
	...
	private static void testServer(int port, Tester<TestEnigmaServer> tester) {
		final TestEnigmaServer server;
		try {
			final EntryRemapper remapper = Enigma.create()
