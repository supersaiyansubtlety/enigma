...
public final class GrepMappingsCommand extends Command<Required, Optionals> {
		...
		final Multimap<ResultType, String> resultsByType = remapper.getMappings().getAllEntries()
				...
				.<Map.Entry<ResultType, String>>mapMulti((obf, add) -> {
					if (obf instanceof ClassEntry obfClass) {
						...
						{ // approach 1 doesn't see constructors
							{//DEBUG TODO
								if (obfMethod.isConstructor()) {
						...
						{ // approach 2 has no way to get the TypeDescriptor of the local
							// DEBUG TODO
							// unlike other entries, locals only have TypeDescriptors on their def entries
							// entryIndex doesn't have def lookup for locals
							// and this is always false
							if (obfParam instanceof LocalVariableDefEntry def) {
								paramFinder.flatMap(finder -> finder.apply(def)).ifPresent(add);
							}
						}
					}
				})
	...
	private static String getName(TypeDescriptor desc, EntryRemapper remapper) {
		if (desc.isVoid()) {
			return "void";
			...
			return getName(desc.getArrayType(), remapper) + "[]".repeat(desc.getArrayDimension());
		} else {
					...
					return Optional.of(obf -> {
						if (
							...
							remapper.getMapping(obf).targetName() != null
								&& typePattern.matcher(getName(descriptorGetter.apply(obf), remapper)).find()
						) {
				...
				return Optional.of(obf -> {
					if (remapper.getMapping(obf).targetName() != null) {
						...
						if (pattern.matcher(deobf.getName()).find()) {
							if (typePattern.matcher(getName(descriptorGetter.apply(obf), remapper)).find()) {
